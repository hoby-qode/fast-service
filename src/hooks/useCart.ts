    ...
    const handleActive = (id: string) => {
      ...
      if (currentUser.username) {
        ...
        if (localStorage.getItem('idsProducts-'+currentUser.id)?.includes(id)) {
          const dataSplit = localStorage.getItem('idsProducts-'+currentUser.id)?.split(',')
          localStorage.setItem('idsProducts-'+currentUser.id, dataSplit!.filter(e => e != id).join(','))
          setIsActive(false)
          setNbCart(localStorage.getItem('idsProducts-'+currentUser.id)?.split(','))
          ...
          if (lastData) {
            localStorage.setItem('idsProducts-'+currentUser.id, lastData + ',' + id)
          } else {
